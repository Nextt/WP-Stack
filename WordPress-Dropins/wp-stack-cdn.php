...
class WP_Stack_CDN_Plugin extends WP_Stack_Plugin {
	...
	public function filter_uploads_only( $content ) {
		...
		return preg_replace( "#=([\"'])(https?://{$domain})?$preg_path/((?:(?!\\1]).)+)\.(" . implode( '|', $this->extensions ) . ")(\?((?:(?!\\1).)+))?\\1#", '=$1http://' . $this->cdn_domain . $path . '/$3.$4$5$1', str_replace('><', '>'.PHP_EOL.'<', $content) );
	}

	public function filter( $content ) {
		return preg_replace( "#=([\"'])(https?://{$this->site_domain})?/([^/](?:(?!\\1).)+)\.(" . implode( '|', $this->extensions ) . ")(\?((?:(?!\\1).)+))?\\1#", '=$1http://' . $this->cdn_domain . '/$3.$4$5$1', str_replace('><', '>'.PHP_EOL.'<', $content) );
	}
